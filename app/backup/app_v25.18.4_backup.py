...
{% if not item %}<div class='panel'><h2>🇦🇺 BAM Market Valuation</h2><div class='muted'>Use the details entered above to search the Australian market. Then enter the comparable asking prices in the Market Valuation fields above and click Calculate & Put Into Quick-Sale Value before saving.</div><div class='actions'><button type='button' class='btn good' id='google_market_valuation_btn'>🔎 Google Market Valuation ↗</button></div></div>{% endif %}
